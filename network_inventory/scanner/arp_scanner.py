...
TCP_SWEEP_PORTS = [
    80, 443, 445, 22, 135, 139, 8080, 5353, 5555, 62078, 8008, 8009, 1900, 3000, 5000, 554
]
...
def _system_ping(ip: str, timeout_sec: float = 1.0) -> bool:
    try:
        ...
        out = subprocess.check_output(args, stderr=subprocess.STDOUT, timeout=timeout_sec + 0.5)
...
async def tcp_sweep(target: ipaddress.IPv4Network, known_ips: set[str], logger: logging.Logger) -> set[str]:
    ...
    async def check_port(ip: str, port: int):
        async with semaphore:
            if ip in known_ips:
                return
            try:
                _, writer = await asyncio.wait_for(asyncio.open_connection(ip, port), timeout=0.8)
                writer.close()
    ...
    # Probe 2 most common ports for speed
    tasks = []
    for host in target.hosts():
        ip = str(host)
        tasks.append(check_port(ip, 80))
        tasks.append(check_port(ip, 445))
        tasks.append(check_port(ip, 5353)) # mDNS for mobiles
...
async def arp_scan(
    ...
    try:
        ...
        logger.warning("Raw ARP scan failed (probably missing Npcap/Permissions): %s", e)
