...
class ServiceListener:
    ...
    def add_service(self, zeroconf: Zeroconf, type: str, name: str) -> None:
        ...
        if info:
            ...
            self.services.append(MdnsService(
                name=name,
                service_type=type,
                addresses=addresses,
                port=info.port,
...
async def discover_mdns(logger: logging.Logger, timeout: float = 3.0) -> list[MdnsService]:
    ...
    browsers = [ServiceBrowser(zeroconf, srv, listener) for srv in COMMON_SERVICES]
